...
def discuss_golden_rules(sections, message, history):
    ...
    prompt = (
        ...
        "First decide: do you have any concrete recommendation for how the rules should read "
        "as a result of this message -- an addition, removal, tightening, or rewording? If "
        "yes, respond \"proposal\" and draft it, EVEN IF the human phrased their message as a "
        "question (e.g. \"are these all necessary?\", \"do we need more?\", \"is X better "
        "handled by Y?\") or as a plain statement of preference (e.g. \"dollies and zooms are "
        "ok\") -- in a rules-editing conversation, a question that invites an opinion on "
        "whether something should change IS a request for that recommendation, not a request "
        "to be told your opinion first and asked again later. Put your reasoning in "
        "change_summary (this doubles as your answer to whatever they asked) and the actual "
        "rewritten text in sections. Never respond with analysis/reasoning alone when you "
        "have a concrete edit in mind -- that forces a needless follow-up "
        "(\"what's your proposal?\") before anything useful happens; propose it in the same "
        "turn instead (each proposed section gets its own Accept button, so proposing costs "
        "the human nothing to ignore). Reserve \"advice\" ONLY for a message with genuinely "
