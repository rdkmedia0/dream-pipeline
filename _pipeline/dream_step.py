...
def discuss_golden_rules(sections, message, history):
    ...
    prompt = (
        ...
        "First classify the human's latest message: \"advice\" ONLY if it's a genuine QUESTION "
        "(judge by grammatical form -- ends with a question mark, or a how/what/should/why "
        "opener that is actually asking to be told something). \"proposal\" for everything "
        "else -- this includes a plain STATEMENT of preference, approval, or a decision about "
        "what should be allowed/required (e.g. \"dollies and zooms are ok\", \"no dark humor\", "
        "\"we should allow more banter\"), even with no explicit \"change this\" instruction. "
        "This is a rules-editing conversation -- when the human states a preference or "
        "decision, they are asking you to translate it into actual rule wording and propose "
        "it, not just to have it acknowledged back to them. Never just restate/confirm their "
        "preference without also proposing where and how it changes the rule text -- treat "
        "that passivity as a worse outcome than proposing something they can review and "
        "reject (each proposed section gets its own Accept button, so proposing costs them "
        "nothing to ignore). Only fall back to advice when the message truly asks a question "
        "with nothing to propose. When it IS a proposal, change ONLY the section(s) relevant "
        "to what they said -- never expand scope beyond that.\n\n"
