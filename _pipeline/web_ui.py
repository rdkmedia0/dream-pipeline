...
function uploadTemplateSection(template, error) {
  ...
  return `
    <div class="card">
      ...
      <p class="m-0 mb-5 muted">Hover any <span class="mf-help">?</span> for what a field does and a link to YouTube's own page for its current rules -- YouTube changes these, the links stay current.</p>
      <label>YouTube channel handle <span class="mf-help" title="The @handle of the YouTube channel this project uploads to. Find it in YouTube Studio > Customisation > Basic info, or at the top of the channel page. Used as a safety check: uploads are refused if the Google account you connect belongs to a different channel, so a video can never land on the wrong one.">?</span> <input id="ut-channel_handle" value="${esc(t.channel_handle)}"></label>
      <label>Episode label (e.g. Tale, Dream) <span class="mf-help" title="The word used in output folder and file names and in every video title, as '<label> #N <title>' (e.g. 'Tale #12 The Otter'). Changing it later renames nothing already rendered or uploaded, so pick it once.">?</span> <input id="ut-episode_label" value="${esc(t.episode_label)}"></label>
      <label>YouTube category ID <span class="mf-help" title="YouTube's numeric video category, set through the API only (Studio shows the name, not the number). Common IDs: 1 Film & Animation, 15 Pets & Animals, 22 People & Blogs, 23 Comedy, 24 Entertainment, 27 Education, 28 Science & Technology. Current full list per region: https://developers.google.com/youtube/v3/docs/videoCategories/list">?</span> <input id="ut-category_id" value="${esc(t.category_id || '24')}"></label>
      <label>Default privacy status <span class="mf-help" title="Applies only to numbers NOT covered by the schedule below. Scheduled numbers always upload as Private with a publish time, and YouTube switches them to Public itself at that moment. What each status means: https://support.google.com/youtube/answer/157177">?</span> <select id="ut-privacy_status">
      ...
      <label>Default language <span class="mf-help" title="Language of the video's title and description as a BCP-47 code ('en', 'en-GB', 'de'). Helps YouTube pick caption language and show the video to the right audience. https://support.google.com/youtube/answer/4571261">?</span> <input id="ut-default_language" value="${esc(t.default_language || 'en')}"></label>
      <label class="gap-4 row"><input type="checkbox" id="ut-made_for_kids" class="w-auto" ${t.made_for_kids ? 'checked' : ''}> Made for kids <span class="mf-help" title="YouTube's legally required 'made for kids' (COPPA) audience setting, applied to every upload. Content made for kids gets no comments, notifications or personalised ads; getting it wrong can mean fines from the FTC. Base it on your actual content, not your preference. YouTube's guidance: https://support.google.com/youtube/answer/9528076">?</span></label>
      <label class="gap-4 row"><input type="checkbox" id="ut-contains_synthetic_media" class="w-auto" ${t.contains_synthetic_media ? 'checked' : ''}> Contains synthetic (AI) media
        <span class="mf-help" title="YouTube's mandatory AI-content disclosure. YouTube asks whether AI (1) made a real person appear to say or do something they didn't, (2) altered footage of a real event or place, or (3) generated a realistic-looking scene that didn't occur -- realistic AI voices count too. Photorealistic AI video of fictional events is case 3: tick it. Clearly animated or stylised content may leave it unticked. When in doubt, tick it: the label costs nothing, a wrong 'no' can cost the channel. Run Update metadata to push a change to videos already uploaded. Current policy: https://support.google.com/youtube/answer/14328491">?</span></label>
      <label>Description footer (appended to every upload's description) <span class="mf-help" title="Text added after every video's own description and before the Dream Pipeline credit: a series tagline, a subscribe line, links (write them with https:// so YouTube makes them clickable). Descriptions are capped at 5,000 characters in total. https://support.google.com/youtube/answer/12340300">?</span> <textarea id="ut-description_footer">${esc(t.description_footer)}</textarea></label>
      ...
      <label>Model named in the credit (leave blank to detect it from the render workflow) <span class="mf-help" title="Only for overriding the model name the credit detects from the render workflow (e.g. you render with a checkpoint it doesn't recognise). Leave blank in the normal case.">?</span> <input id="ut-credit_model" value="${esc(t.credit_model || '')}" placeholder="auto-detected, e.g. LTX"></label>
      <label>Default tags (comma-separated) <span class="mf-help" title="Added to every video after that video's own spec tags. Tags are never shown to viewers and carry little search weight; the total is capped at 500 characters. https://support.google.com/youtube/answer/146402">?</span> <input id="ut-default_tags" value="${esc(tags)}"></label>
      <h4>Schedule</h4>
      <label class="gap-4 row"><input type="checkbox" id="ut-schedule_enabled" class="w-auto" ${sch.enabled !== false ? 'checked' : ''}> Enabled <span class="mf-help" title="On: every upload gets a publish time computed from the anchor below, goes up as Private, and YouTube publishes it automatically at that time. Off: uploads use the Default privacy status above immediately. Scheduled publishing on YouTube: https://support.google.com/youtube/answer/9012226">?</span></label>
      ...
      <label>Anchor number (the video number the schedule counts from) <span class="mf-help" title="The video number that publishes on the anchor date. Every later number takes the next slot in the days-of-week rota below, in numeric order, regardless of when you actually upload it.">?</span> <input id="ut-schedule_anchor_number" value="${esc(sch.anchor_number ?? 1)}"></label>
      <label>Anchor date (YYYY-MM-DD) <span class="mf-help" title="Publication date of the anchor number, YYYY-MM-DD, in the timezone below. Slots are counted forward from here. YouTube refuses a publish time in the past, so an old anchor with many videos already out is fine only if the numbers still to come land on future dates.">?</span> <input id="ut-schedule_anchor_date" value="${esc(sch.anchor_date)}"></label>
      <label>Days of week it publishes on (comma-separated) <span class="mf-help" title="Weekdays that get a slot, comma-separated full names, e.g. Monday, Thursday. Order doesn't matter.">?</span> <input id="ut-schedule_days" value="${esc(days)}"></label>
      <label>Time of day (HH:MM:SS, local) <span class="mf-help" title="Local time of day each slot publishes at, 24-hour HH:MM:SS, converted to UTC for YouTube with the timezone below. Studio's own audience reports (YouTube Studio > Analytics > Audience > 'When your viewers are on YouTube') are the place to check what time suits your channel.">?</span> <input id="ut-schedule_time_of_day" value="${esc(sch.time_of_day_local || '00:00:00')}"></label>
      <label>Timezone <span class="mf-help" title="IANA timezone name the anchor date and time of day are given in, e.g. Europe/Zurich, America/New_York, Asia/Tokyo. Full list: https://en.wikipedia.org/wiki/List_of_tz_database_time_zones">?</span> <input id="ut-schedule_timezone" value="${esc(sch.timezone || 'Europe/Zurich')}"></label>
